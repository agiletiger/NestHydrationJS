...
function nestHydrationJS() {
	...
	var isFunction = (a) => !!a && a.constructor === Function;
	var isPlainObject = (a) => !!a && a.constructor === Object;
	...
	NestHydrationJS.nest = function (data, structPropToColumnMap) {
		...
		if (!Array.isArray(structPropToColumnMap) && !isPlainObject(structPropToColumnMap) && structPropToColumnMap !== null && structPropToColumnMap !== true) {
		...
		if (structPropToColumnMap === null && table.length > 0) {
			...
			structPropToColumnMap = NestHydrationJS.structPropToColumnMapFromColumnHints(Object.keys(table[0]));
		}
	...
	NestHydrationJS.buildMeta = function (structPropToColumnMap) {
		...
		_buildMeta = function (structPropToColumnMap, isOneOfMany, containingColumn, ownProp) {
			...
			propList = Object.keys(structPropToColumnMap ? structPropToColumnMap : {});
			...
			for (i = 0; i < propList.length; i++) {
				...
				if (typeof structPropToColumnMap[prop] === 'string') {
					...
					subIdColumn = Object.values(structPropToColumnMap[prop])[0];
		...
		if (Array.isArray(structPropToColumnMap)) {
			...
			primeIdColumn = Object.values(structPropToColumnMap)[0];
				...
				if (nav === '') {
					...
					if (typeof pointer[prop][nav] === 'undefined') {
						...
						if (isId) {
							...
							prevKeyList = Object.keys(pointer[prop]);
